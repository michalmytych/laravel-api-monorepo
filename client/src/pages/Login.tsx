      ...
      <form onSubmit={handleSubmit} noValidate>
        <div>
          ...
          <input id="login-email" type="email" autoComplete="email" value={email}
            onChange={updateField(setEmail)} aria-invalid={Boolean(fieldErrors.email)}
            aria-describedby={fieldErrors.email ? "login-email-error" : undefined} />
          {fieldErrors.email?.map((message) => <span id="login-email-error" key={message}>{message}</span>)}
        </div>
        <div>
          ...
          <input id="login-password" type="password" autoComplete="current-password" value={password}
            onChange={updateField(setPassword)} aria-invalid={Boolean(fieldErrors.password)}
            aria-describedby={fieldErrors.password ? "login-password-error" : undefined} />
          {fieldErrors.password?.map((message) => <span id="login-password-error" key={message}>{message}</span>)}
        </div>
